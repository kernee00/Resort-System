...
<body>
    ...
    <table class="table" border="2" cellspacing="7">
        ...
        // except the first admin who has the highest privileges
            $query="SELECT  * FROM  resorts Where ;" ;
        ...
        if(mysqli_num_rows($result)>=1){
            while ($row=$result->fetch_assoc()) {
                ...
                <td><a href = 'updateResort.php?resortID=$row[resortID] & address = $row[address] & contact = $row[resortPhoneNo] & ratings = $row[overallRatings] & price = $row[pricePerNight] & keyword = $row[keywords]'><input type = 'submit' value = 'Edit' id = 'button'></a></td>
